  ...
  return (
    <div className="min-h-screen pt-20 bg-parchment">
      ...
      >
        <div className="max-w-6xl mx-auto text-center">
          ...
          >
            ...
            Stay informed with expert insights and cutting-edge research.
          </motion.p>
        </div>
      </motion.section>
          ...
          >
            {categories.map((category, index) => (
              <button
                ...
                className={`px-4 py-2 rounded-full font-sans-body font-medium transition-all duration-300 ${
      ...
      {selectedCategory === 'All' && (
        <section className="pb-16 px-4 sm:px-6 lg:px-8 bg-parchment">
          <div className="max-w-6xl mx-auto">
            <motion.div
              initial={{ opacity: 0, y: 50 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8 }}
              className="vintage-card rounded-lg shadow-vintage overflow-hidden mb-16"
            >
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                <div className="relative h-64 lg:h-auto overflow-hidden">
                  <img
                    ...
                    className="w-full h-full object-cover hover:scale-105 transition-transform duration-500 member-image"
                  />
                  <div className="absolute top-4 left-4">
                    <span className="bg-gradient-to-r from-olive to-muted-gold text-paper px-3 py-1 rounded-full text-xs font-bold">
                      FEATURED
                    </span>
                  </div>
                </div>
                ...
                <div className="p-8 lg:p-12 flex flex-col justify-center">
                  <div className="flex items-center mb-4 text-sm text-technical-gray">
                    <Tag className="w-4 h-4 mr-2 text-olive" />
                    <span className="font-sans-body">{articles[0].category}</span>
                  </div>
                  
                  <h2 className="font-serif-heading font-bold text-2xl md:text-3xl text-forest mb-4 leading-tight">
                    {articles[0].title}
                  </h2>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center text-sm text-technical-gray">
                      <User className="w-4 h-4 mr-2 text-vintage-blue" />
                      <span className="font-sans-body mr-4">{articles[0].author}</span>
                      <Calendar className="w-4 h-4 mr-2 text-olive" />
                      <span className="font-sans-body mr-4">{new Date(articles[0].date).toLocaleDateString()}</span>
                      <Clock className="w-4 h-4 mr-2 text-brown" />
                      <span className="font-sans-body">{articles[0].readTime}</span>
                    </div>
                  </div>
                  ...
                  <motion.a
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="claymorphic-button inline-flex items-center space-x-2 px-6 py-3 rounded-full font-serif-heading font-semibold hover:from-brown hover:to-olive transition-all duration-300 w-fit"
                  >
              ...
              >
                <div className="vintage-card rounded-lg shadow-vintage hover:shadow-ink-stamp transition-all duration-300 overflow-hidden h-full">
                  <div className="relative h-48 overflow-hidden">
                    ...
                    />
                    <div className="absolute top-4 left-4">
                      <span className="bg-vintage-blue/20 text-vintage-blue px-2 py-1 rounded text-xs font-medium border border-vintage-blue/30">
                        {article.category}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-6">
                    <h3 className="font-serif-heading font-bold text-lg text-forest mb-3 group-hover:text-olive transition-colors leading-tight">
                      {article.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between text-xs text-technical-gray mb-4">
                      <div className="flex items-center">
                        <User className="w-3 h-3 mr-1 text-olive" />
                        <span className="font-sans-body mr-3">{article.author}</span>
                      </div>
                      <div className="flex items-center">
                        <Calendar className="w-3 h-3 mr-1 text-vintage-blue" />
                        <span className="font-sans-body mr-3">{new Date(article.date).toLocaleDateString()}</span>
                        <Clock className="w-3 h-3 mr-1 text-brown" />
                        <span className="font-sans-body">{article.readTime}</span>
                      </div>
                    </div>
                    
                    <a
                      href={article.link}
                      className="inline-flex items-center space-x-2 text-olive hover:text-vintage-blue transition-colors font-sans-body font-medium text-sm"
                    >
                      <span>Read More</span>
          ...
          <div className="vintage-card p-12 rounded-lg shadow-vintage">
            ...
            <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
              <input
                ...
                className="flex-1 px-4 py-3 bg-paper border border-olive/30 rounded-lg focus:outline-none focus:border-olive text-ink font-sans-body"
              />
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="claymorphic-button font-serif-heading font-semibold px-6 py-3 rounded-lg hover:from-brown hover:to-olive transition-all duration-300"
              >
                Subscribe
              </motion.button>
            </div>
